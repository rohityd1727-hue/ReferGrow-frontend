...
export default function DashboardPage() {
  ...
  async function loadAll() {
    ...
    const meBody = await readBody(meRes);
    if (!meRes.ok) throw new Error(responseErrorMessage(meRes, meBody));

    const servicesBody = await readBody(servicesRes);
    const incomeBody = await readBody(incomeRes);

    const meJson = meBody.json as { user?: MeResponse["user"] } | null;
    const servicesJson = servicesBody.json as { services?: Service[] } | null;
    const incomeJson = incomeBody.json as { incomes?: Income[] } | null;
